...
base {ℓ = ℓ} {k = k} {j = j} {X = X} {I = I} B Bcovers Bbase =
  ...
  where
    ...
    B-⊆ x∈U OpenU y = proj₁ (proj₂ (OpenU x∈U)) y

    ∈-B : {x : X} {U : ℙ j X} (x∈U : x ∈ U) (OpenU : Open' U) → x ∈ B (index-of x∈U OpenU)
    ∈-B x∈U OpenU = proj₂ (proj₂ (OpenU x∈U))

    B-index-of : ∀ {i j x} → x ∈ B i → x ∈ B j → I
    B-index-of x∈Bi x∈Bj = proj₁ (Bbase x∈Bi x∈Bj)

    B-⊆-∩ : ∀ {i j x} → (x∈Bi : x ∈ B i) → (x∈Bj : x ∈ B j) → B (B-index-of x∈Bi x∈Bj) ⊆ B i ∩ B j
    B-⊆-∩ x∈Bi x∈Bj = proj₂ (proj₂ (Bbase x∈Bi x∈Bj))

    B-∈-B : ∀ {i j x} → (x∈Bi : x ∈ B i) → (x∈Bj : x ∈ B j) → x ∈ B (B-index-of x∈Bi x∈Bj)
    B-∈-B x∈Bi x∈Bj = proj₁ (proj₂ (Bbase x∈Bi x∈Bj))

    ∩-open' : (U V : ℙ j X) → Open' U → Open' V → Open' (U ∩ V)
    ∩-open' U V OpenU OpenV (x∈U , x∈V) =
     (B-index-of (∈-B x∈U OpenU) (∈-B x∈V OpenV)) ,
     (λ y y∈B → (B-⊆ x∈U OpenU y (∩-⊆-left (B (index-of x∈U OpenU)) (B (index-of x∈V OpenV)) y (B-⊆-∩ (∈-B x∈U OpenU) (∈-B x∈V OpenV) y y∈B))) ,
                (B-⊆ x∈V OpenV y (∩-⊆-right (B (index-of x∈U OpenU)) (B (index-of x∈V OpenV)) y (B-⊆-∩ (∈-B x∈U OpenU) (∈-B x∈V OpenV) y y∈B)))
     ) ,
